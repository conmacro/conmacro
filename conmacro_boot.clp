...
(deffacts gen2 (file-extension (is "clp") (assumed-type clips-source-code)))
(deffacts gen3 (file-extension (is "md") (assumed-type markdown)))
...
(file-content (file ?file) (is ?content))
 ...
 (bind ?logicalName (gensym))
    ...
    (close ?name))
(deftemplate section 
(slot level (default 1))
(slot title)
(multislot is)
)
(defmethod to-verbatim ((?f FACT-ADDRESS (and (eq (fact-relation ?f) section) (> (fact-slot-value ?f level) 0) (< (fact-slot-value ?f level) 7))) (?type (eq ?type markdown)) )
(bind ?h "")
...
(defmethod to-verbatim ((?f FACT-ADDRESS (eq (fact-relation ?f) ol)) (?type (eq ?type markdown)) )
(bind ?h "")
...
(defmethod to-verbatim ((?f FACT-ADDRESS (eq (fact-relation ?f) link)) (?type (eq ?type markdown)) )
(format nil "[%s](%s)" (fact-slot-value ?f name) (fact-slot-value ?f to)))
